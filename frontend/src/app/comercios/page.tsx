...
// Indexa el directorio base y mantiene las combinaciones de filtros fuera del índice.
export async function generateMetadata({ searchParams }: Props) {
  ...
  for (const clave of Object.keys(parametros).sort()) {
    ...
    for (
      const entrada of Array.isArray(valor)
        ? valor
        : valor === undefined
          ? []
          : [valor]
    ) {
      query.append(clave, entrada);
    }
  }

  const tieneFiltros = query.size > 0;

  return crearMetadata(
    "Buscar negocios por nombre, categoría y municipio",
    "Busca en el directorio de ProxiMolar y combina filtros para encontrar un negocio. Consulta sus datos y contacta directamente.",
    tieneFiltros ? `/comercios?${query}` : "/comercios",
    !tieneFiltros
  );
}
